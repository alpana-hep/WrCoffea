...
def make_skimmed_events(events):
    ...
    skimmed_dropped = skimmed[list(set(x for x in skimmed.fields if x in ["Electron", "Muon", "Jet", "HLT.Ele32_WPTight_Gsf","HLT.Photon200", "HLT.Ele115_CaloIdVT_GsfTrkIdT", "HLT.Mu50", "HLT.OldMu100", "HLT.TkMu100", "event"]))]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start', type=int, default=0, help='File number at which to start')
...
#    client = Client(cluster)
    ...
    for i in range(args.start, num_files):
        t0 = time.monotonic()
        ...
        with ProgressBar():
            for dataset, skimmed in skimmed_dict.items():
                ...
                uproot.dask_write(skimmed, compute=True, destination="dataskims/", prefix=f"{args.dataset}_{args.year}_{args.run}/{args.dataset}{args.year}{args.run}_file{i+1}", tree_name = "Events")
