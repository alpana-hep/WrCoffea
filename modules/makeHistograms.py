...
#        self.event_count = hda.Hist.new.Reg(1, 0, 1, label=r"Event Count").Weight()

        self.event_weight = hda.Hist.new.Reg(1, 0, 1, label=r"Event Weight").Weight()
        ...
        self.pt_dileptons = hda.Hist.new.Reg(200, 0, 1000, label=r"p^{T}_{ll} [GeV]").Weight()
    ...
    def eventweights(self, events):
        self.event_weight.fill(events.eventWeight)

    def FillHists(self, events, weights=None):
#        self.event_count.fill(0.5, weight = weights)
        ...
        self.pt_dileptons.fill((events.leptons[:, 0]+events.leptons[:, 1]).pt, weight = weights)
