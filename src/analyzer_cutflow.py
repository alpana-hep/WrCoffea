...
#            print(lumi.compute())
        ...
        regions = {
#            'WR_EE_Resolved_DYCR': ['twoTightLeptons',  'leadTightLeptonPt60', 'eeTrigger', 'minTwoAK4Jets',  'eejj', 'dr>0.4', 'mlljj>800', '60mll150'],
            'WR_MuMu_Resolved_DYCR': ['twoTightLeptons',   'leadTightLeptonPt60', 'mumuTrigger', 'minTwoAK4Jets', 'mumujj', 'dr>0.4', 'mlljj>800', '60mll150'],
        }

        
        # ——— Print an actual cutflow table for each region ———
        for region_name, cut_list in regions.items():
            # build the Cutflow object (this uses your Cutflow class under the hood)
            cf = selections.cutflow(
                *cut_list,
            )
            print(f"\n===== Cutflow for {region_name} =====")
            cf.print()   # this will print pass / cumulative / efficiency
