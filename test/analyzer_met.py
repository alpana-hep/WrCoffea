...
#            'pt_fourobject':          self.create_hist('pt_fourobject',          'process', 'region', (800,   0, 8000), r'$p_{T,\ell\ell jj}$ [GeV]'),
            ...
            ### NEW: 1D MET
            'met':                    self.create_hist('met',                    'process', 'region', (200, 0, 1000), r'$E_{T}^{\text{miss}}$ [GeV]'),

            ### NEW: 2D MET vs mll (for MC, mlljj > 1.5 TeV)
            'met_vs_mll_1p5TeV':     self.create_hist2d('met', 'mass_dilepton', 'process', 'region',
                                                        (200, 0, 1000), r'$E_{T}^{\text{miss}}$ [GeV]',
                                                        (200, 0, 2000), r'$m_{\ell\ell}$ [GeV]'),
        }
    ...
    ### NEW: helper for 2D histograms
    def create_hist2d(self, name_x, name_y, process, region, bins_x, label_x, bins_y, label_y):
        return (
            hist.Hist.new.StrCat([], name="process", label="Process", growth=True)
            .StrCat([], name="region",  label="Analysis Region", growth=True)
...
#            ('phi_subleading_jet',        jets[:, 1].phi,      'phi_subleadjet'),
            ('mass_dilepton',             mll_array,           'mass_dilepton'),
...
#            ('pt_fourobject',             (leptons[:, 0] + leptons[:, 1] + jets[:, 0] + jets[:, 1]).pt,   'pt_fourobject'),
        ...
        # If an SF variable is configured, cache the full (uncut) values for corr lookup
        ...
        # --- Fill 1D hists (incl. MET)
        for hist_name, values, axis_name in variables:
            vals = values[cut]
            w = weights.weight()[cut]

            # Optional DY control/signal region SFs
            if process_name == "DYJets" and self.lookup_EE is not None and self.variable is not None and vals_all is not None:
        ...
        # --- NEW: 2D MET vs mll, only for MC and with mlljj > 1.5 TeV
        if is_mc:
            hi_mass = (mlljj_array > 1500.0)
            cut2d = cut & ak.values_astype(hi_mass, bool)

            if ak.any(cut2d):
                xvals = met_pt[cut2d]         # MET
                yvals = mll_array[cut2d]      # mll
                w2 = weights.weight()[cut2d]

                # Apply optional DY SFs consistently
                if process_name == "DYJets" and self.lookup_EE is not None and self.variable is not None and vals_all is not None:
                    if region.startswith("wr_ee_resolved_dy_cr") or region.startswith("wr_ee_resolved_sr"):
                        corr2 = self.lookup_EE(vals_all[cut2d])
                    elif region.startswith("wr_mumu_resolved_dy_cr") or region.startswith("wr_mumu_resolved_sr"):
                        corr2 = self.lookup_MM(vals_all[cut2d])
                    else:
                        corr2 = 1.0
                    w2 = w2 * corr2

                output['met_vs_mll_1p5TeV'].fill(
                    process=process_name,
                    region=region,
                    met=xvals,
                    mass_dilepton=yvals,
...
#            'wr_mumu_resolved_sr': ['twoTightLeptons', 'minTwoAK4Jets', 'leadTightLeptonPt60', 'mumuTrigger', 'mlljj>800', 'dr>0.4', '400mll', 'mumujj'],
            'wr_ee_resolved_sr': ['twoTightLeptons', 'minTwoAK4Jets', 'leadTightLeptonPt60', 'eeTrigger', 'mlljj>1500', 'dr>0.4','eejj', '150mll'],
            'wr_mumu_resolved_sr': ['twoTightLeptons', 'minTwoAK4Jets', 'leadTightLeptonPt60', 'mumuTrigger', 'mlljj>1500', 'dr>0.4', 'mumujj', '150mll'],
        }
        ...
        for region, cuts in regions.items():
            ...
            self.fill_basic_histograms(
                output, region, cut, process_name,
                AK4Jets, tightLeptons, weights,
                met_pt, mll, mlljj,
                is_mc=(not isRealData)  # NEW: fill 2D only for MC
            )
